  ...
  <div class="min-h-screen bg-white dark:bg-slate-900 text-slate-900 dark:text-white">
    ...
    <main class="max-w-6xl mx-auto px-4 py-12">
      ...
      <table v-else class="w-full table-auto border-collapse">
        <thead>
          <tr class="bg-slate-100 dark:bg-slate-800 text-left text-sm uppercase text-slate-600 dark:text-slate-400">
            <th class="p-3">Nom</th>
        ...
        <tbody>
          <tr v-for="user in filteredUsers" :key="user.id" class="border-b border-slate-200 dark:border-slate-700">
      ...
      <div v-if="userToDelete" class="fixed inset-0 bg-black/50 flex items-center justify-center z-50">
        <div class="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-xl max-w-md w-full">
          ...
          <p class="mb-6">Êtes-vous sûr de vouloir supprimer l'utilisateur <strong>{{ userToDelete.email }}</strong> ? Cette action est irréversible.</p>
          <div class="flex justify-end gap-3">
            <button @click="userToDelete = null" class="px-4 py-2 bg-gray-300 dark:bg-slate-700 text-slate-800 dark:text-white rounded">Annuler</button>
            <button @click="deleteUserConfirmed" class="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700">Supprimer</button>
          </div>
        </div>
      </div>
    </main>
